# プログラミング実習 課題3：CLIタスク管理ツール「my-todo」SQLite編
...
データの保存は **sqlite3** を使用しています。
また、初回起動時にtodos.dbの作成とタスクを保存するtasksテーブルが作成されます。
追加したタスクはSQLiteデータベース (todos.db)に保存されます。
